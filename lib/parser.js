...
function throwError(buffer, index, message) {
    ...
    throw new SyntaxError(errMessage);
}

function parseError(mode, tree, index, length, buffer, indent) {
    throwError(buffer, index - 1, 'Unexpected token: ' + JSON.stringify(buffer[index])+'.');
}
...
function parseTagEnd(mode, tree, index, length, buffer, indent, close) {
    ...
    if (ch === 62 /*'>'*/) {
        compile.debug && console.log(indent + 'parseTagEnd');
        close.closed = true;
        return index;
    }
        ...
        if (ch === 61) {
            ...
            if (ch === 34 || ch === 39) {
                ...
                if (!stringToken.closed) {
                    throwError(buffer, [startIndex, index], 'Missing closing tag: expected \'' + stringToken + '\'.');
                }
            } else {
                var textValueToken = {
                    type: 'TEXT',
                ...
                if (textValueToken.text) {
                    ...
                    throwError(buffer, [startIndex, index], 'Unexpected end of input.');
                }
            }
        } else {
            index--;
...
function parse(mode, tree, index, length, buffer, indent, close) {
    compile.debug && console.log(indent + 'parse - ', mode);

    // compile.debug && console.log({mode: mode, tree: tree, index: index, length: length, buffer: buffer, close: close, indent: indent});
...
function parseTag(mode, tree, index, length, buffer, indent) {
    compile.debug && console.log(indent+'parseTag');
...
function parseTagClose(mode, tree, index, length, buffer, indent, close, noErrorOnMismatch) {
    ...
    compile.debug && console.log(indent+'parseTagClose');
    ...
    if (!close) {
        throwError(buffer, [startIndex, index], 'Unexpected closing tag: \'' +token.name+ '\'.');
    }
    ...
    if (token.type === close.type && token.name === close.name) {
        ...
        throwError(buffer, [startIndex, index], 'Mismatched closing tag: expected \'' +close.name+ '\' but found \'' +token.name+ '\'.');
    }
        ...
        if (ch === 125 /*'}'*/) {
            ...
            for (; index < length; index++) {
                ...
                if (ch === 125 /*'}'*/) {
                    ...
                    throwError(buffer, index - 1, 'Unexpected character: expected \'}\' but found \'' +buffer[index]+ '\'.');
                }
...
function parseBarsPartial(mode, tree, index, length, buffer, indent) {
    ...
    if (!token.name) {
        throwError(buffer, [startIndex, index], 'Missing partial name.');
    }
...
function parseBarsHelper(mode, tree, index, length, buffer, indent) {
    ...
    if (!token.name) {
        throwError(buffer, [startIndex, index], 'Missing helper name.');
    }
...
function parseBarsHelperHTML(mode, tree, index, length, buffer, indent) {
    ...
    compile.debug && console.log(indent+'parseBarsHelperHTML');
...
function parseBarsBlock(mode, tree, index, length, buffer, indent) {
    ...
    if (!token.name) {
        throwError(buffer, [startIndex, index], 'Missing block name.');
    }
...
function parseBarsBlockClose(mode, tree, index, length, buffer, indent, close, noErrorOnMismatch) {
    ...
    if (buffer.codePointAt(index + 1) !== 123 /*'{'*/) {
        throwError(buffer, startIndex, 'Unexpected end of input.');
    }
...
function parseBarsBlockElse(mode, tree, index, length, buffer, indent, close) {
    ...
    if (close && close.type === 'BLOCK' && name === 'else') {
        if (close.elsed) {
            throwError(buffer, [startIndex, index], 'Unexpected else token.');
        }
        ...
        throwError(buffer, [startIndex, index], 'Unexpected else tag.');
    } else {
        /* Canceling Parse */
        return null;
    }
...
function compile(buffer) {
    ...
    compile.debug && console.log('compile');

    parse('DOM-MODE', tree.nodes, 0, buffer.length, buffer, '  ', null);

    compile.debug && console.log('compiled');
    //
    compile.debug && console.log(Date.now()-n);
