...
function STRING(mode, code, tokens, close) {
    ...
    if (index > code.index) {
        ...
        if (
            close.type === 'UNARY-EXPRESSION' ||
            close.type === 'BINARY-EXPRESSION'
        ) {
            close.close(code);
        }
        ...
        loop: for (; index < length; index++) {
            ...
            if (48 <= ch && ch <= 57) {
                ...
                if ( /* [+-]?[0-9] */
                    ...
                    code.index = index - 1;
                    throw code.makeError('Unexpected Token: "e".');
                }
...
function EXPRESSION(mode, code, tokens, close) {
    ...
    if ( /* handle UNARY-EXPRESSION */
        ch === 33 /* ! */
    ) {
        expression = new Token(code, 'UNARY-EXPRESSION');
        ...
        expression = new Token(code, 'BINARY-EXPRESSION');
    ...
    if (expression.type === 'BINARY-EXPRESSION') {
        ...
        if (
            expression.left.type !== 'STRING' &&
            expression.left.type !== 'NUMBER' &&
            expression.left.type !== 'UNARY-EXPRESSION' &&
            expression.left.type !== 'BINARY-EXPRESSION'
        ) {
    ...
    if (
        expression.right.type !== 'STRING' &&
        expression.right.type !== 'NUMBER' &&
        expression.right.type !== 'UNARY-EXPRESSION'
    ) {
    ...
    if (expression.type === 'UNARY-EXPRESSION') {
...
function TRANSFORM(mode, code, tokens, close) {
    ...
    transform = new Token(code, 'TRANSFORM');
